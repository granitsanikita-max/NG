...
- **Match the exact emotion Nikita asked for, not just the subject.** "Feet rubbing FROM PAIN" means a
  clip that reads as *pain* (wincing, clutching a sore foot, a red/irritated heel) — NOT a clean
  spa/self-care foot massage. If the query keeps returning the wrong emotion, add the pain/relief/injury
  words explicitly (`wincing`, `sore`, `aching`, `clutching`, `plantar`) and re-search. Showing him a
  technically-matching but emotionally-wrong clip is a miss.
- The script filters hard specs only (≥720 short side, duration window, portrait-first). **You** then
  judge each candidate's `preview_image` against `quality-bar.md`. Read can't fetch a URL here, so
  **download the preview JPGs to a temp dir with `curl` (use `--cacert /root/.ccr/ca-bundle.crt -A
  "Mozilla/5.0"`), then Read the local files.** Drop everything that reads as
  generic/corporate/watermarked/cheap or the wrong emotion.
...
## Step 5 — Present candidates as VIEW links, then download only picks

Present per scene, scannable:
`Scene 3 — "hands squeezing a date": [▸ view] 1080×1920 · 6s · authentic macro, natural light — ✅ hits the bar`
Give the candidate's **view link** — the clip's `page_url` on Pexels/Pixabay so Nikita can actually
WATCH it — plus a one-line why-it-matches + the quality read. **Do NOT auto-download.** He views first,
then tells you which to keep. Only after he picks, download those, named by scene:
