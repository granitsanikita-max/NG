...
WORK = the work folder the lead session set up (it holds formats_image.json, scripts, hunt2/). Your task file is WORK/hunt2/<TASK>_task.json.
TODAY = the date you run this. Goal: for EACH format in your task, deliver AT LEAST 5 (aim 6-7) live static ads that
(a) visually ARE that format and (b) are proven WINNERS. Quality over speed.
Never modify Notion or Miro. Work only inside WORK/hunt2/. Prefix every helper file with your task name (e.g. I3_parse.py)
...
Download `image` (media.winninghunter.com) -> WORK/hunt2/img_<ID>.jpg; convert webp/png to a real RGB JPEG with PIL.
...
## Output: WORK/hunt2/<TASK>.json (update after each format so progress is never lost)
...
While scanning long-running static winners across niches, spot recurring static formats that are NOT in WORK/formats_image.json
